...
fpca_input_path <- paste(path_results, "HER2__domain_input_for_FPCA.RData", sep = "")
...
for (trial_i in 1:n_trials){
  ...
  h <- 1 ## step of the grid
  seed_point <- SpatialPoints(data.frame(x = 11, y = -11)) ## grid seed Point
  ...
  # Meshing
  maximum_area <- 0.1 ## maximum area for a mesh element
