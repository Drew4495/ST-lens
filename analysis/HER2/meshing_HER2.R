...
cat.script_title("HER2 dataset mesh generation")
...
mkdir(c(path_images))
path_images <- paste("images/", name_dataset, "/meshing/", sep = "")
mkdir(c(path_images))
...
if (RUN_plots) {
  plot <- ggplot() +
    ...
    geom_sf(data = st_as_sf(locations), color = "black", size = 1.1)
...
if (RUN_plots) {
  plot <- ggplot() + mesh_generation_plot_settings() +
    ...
    geom_sf(data = st_as_sf(locations), color = "black", size = 1.1) +
    geom_sf(data = st_as_sf(seed_point), color = "red", size = 1.3) +
...
## plot
if (RUN_plots) {
  plot <- plot.final_locations(locations, SpatialPoints(locations.final), lattice_simplified, size = 1.1) +
    mesh_generation_plot_settings() +
    xlim(xmin, xmax) + ylim(ymin, ymax)
...
save(mesh, lattice,
     file = paste(path_data, "mesh.RData", sep = ""))
