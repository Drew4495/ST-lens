...
cat.script_title("Deciding Number of Spatial Components (NSC) for clustering: HER2 dataset")
...
## ||||||||||||||||||||
# Plot Variation explained by # of components ----
## ||||||||||||||||||||

### Redefine variables for easier changes
...
plot <- plot.nComp_selection(residuals_norm, 20, 4, theme_custom = custom_theme)
grid.draw(plot)
...
nComp_opt <- 4      #It looks like 4 is the best. Possibly 3. Will test both
