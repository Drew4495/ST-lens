...
cat.script_title("Plotting DLPFC dataset")
...
load(glue("{directory.results}{name.dataset}_clusters_at_locations.RData"))
...
if(RUN[["Spatial Comps"]]){
  ...
  ggsave(filename=glue("{directory.images}HER2_FPCA_SpatialComps.pdf"), 
  ...
  ggsave(filename=glue("{directory.images}HER2_FPCA_SpatialComps_Scaled.pdf"), 
