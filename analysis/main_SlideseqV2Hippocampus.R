# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# %% Analysis SlideseqV2Hippocampus dataset %%
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
cat.script_title("Analysis SlideseqV2Hippocampus dataset")
...
# Hyper-parameters
h <- 50
bbox <- data.frame(min = c(850, -5650), max = c(5750, -750))
seed_point <- NULL
type = "hexagonal"
simplification <- 0.05
remove_holes <- TRUE
...
# Processed data
locations.prev <- data.frame(locations)
locations.final <- data.frame(locations.final)
locations <- data.frame(locations.final)
names.locations <- rownames(locations)
mesh <- mesh
lattice <- lattice
...
if(PLOT){
  plot <- plot.final_locations(SpatialPoints(locations.prev),
                               SpatialPoints(locations.final), lattice)
...
# HR grid
grid <- square_grid(SpatialPoints(locations)@bbox, 50, seed_point = seed_point)
...
# knearest_vect <- seq(250, 400, by = 10)
...
if(RUN[["Clustering on HR grid"]]){
  ...
  # Save clusters
  save(cluster_labels_HR, cluster_labels_downsamples,
       # Saving options
       file = paste(directory.results, name.dataset, "_clusters_on_HR_grid", ".RData", sep = ""))
  
  # Clean
  # rm(cluster_labels_HR, cluster_labels_refined_HR,
...
# if(PLOT){
#   plot <- plot.field_points(grid, cluster_labels_refined_HR, colormap = "H", size = 0.5, discrete = TRUE) +
...
if(PLOT){
  plot <- plot.field_points(locations, cluster_labels_downsamples, colormap = "H", size = 0.05, discrete = TRUE) +
